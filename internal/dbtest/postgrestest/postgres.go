...
func prepareDB(ctx context.Context, dbPool *pgxpool.Pool, port nat.Port) {
	...
	b.Queue(`INSERT INTO sessions (state_id, tenant_id, fingerprint, token, expiry) VALUES ('stateid-one', current_setting('app.tenant_id'), 'fingerprint-one', 'token-one', $1);`, DBTime)
