...
func TestNewTLSRS(t *testing.T) {
	...
	got, err := NewTLSRS(t.Context(), srv.URL, "client-id", tlsConfig)
	if err != nil {
...
func TestNewTLSRS_DiscoveryError(t *testing.T) {
	if _, err := NewTLSRS(t.Context(), "http://127.0.0.1:0", "client-id", &tls.Config{}); err == nil {
...
func TestNewTLSRP(t *testing.T) {
	...
	tlsConfig := &tls.Config{}
	got, err := NewTLSRP(t.Context(), "client-id", srv.URL, "https://app.example.com/callback", tlsConfig)
	if err != nil {
