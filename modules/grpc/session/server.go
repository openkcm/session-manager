...
func (s *Server) GetSession(ctx context.Context, req *sessionv1.GetSessionRequest) (*sessionv1.GetSessionResponse, error) {
	...
	if g := toStringSlice(groups); g != nil {
		...
		slogctx.Warn(ctx, "missing 'groups' claim; using session-stored groups", "introspection_groups", groups, "session_groups", sess.Claims.Groups)
	}
...
func (s *Server) resourceServer(ctx context.Context, oidcTrust *oidcv1.OIDC) (rs.ResourceServer, error) {
	return s.cProvider.ResourceServer(ctx, oidcTrust.GetClientId(), oidcTrust.GetIssuer())
}
...
func (s *Server) introspectToken(ctx context.Context, token string, oidcTrust *oidcv1.OIDC) (oidc.IntrospectionResponse, error) {
	...
	resServer, err := s.resourceServer(ctx, oidcTrust)
	if err != nil {
		slogctx.Error(ctx, "Could not create resource server for OpenID provider", "issuer", oidcTrust.GetIssuer(), "error", err)
		return oidc.IntrospectionResponse{}, fmt.Errorf("creating resource server: %w", err)
	}
	...
	if intrURL == "" {
		...
		return oidc.IntrospectionResponse{Active: true}, nil
	}
	...
	intr, err := rs.Introspect[oidc.IntrospectionResponse](ctx, resServer, token)
	if err != nil {
		return oidc.IntrospectionResponse{}, fmt.Errorf("executing introspection request: %w", err)
	}
...
// decoded from JSON, so a string array arrives as []any; this coerces
