...
func TestModule_ProvisionInsecure(t *testing.T) {
	...
	creds, err := m.ResourceServer(t.Context(), "client-id", srv.URL)
	require.NoError(t, err)
...
func TestModule_ProvisionClientSecret(t *testing.T) {
	...
	require.NoError(t, err)
	srv := newDiscoveryServer(t)
	creds, err := m.ResourceServer(t.Context(), "cid", srv.URL)
	require.NoError(t, err)
	assert.NotNil(t, creds)
}
...
func TestModule_RelyingPartyInsecure(t *testing.T) {
	...
	srv := newDiscoveryServer(t)
	rp, err := m.RelyingParty(t.Context(), oidcFor(srv.URL, "client-id"), "https://app.example.com/callback")
	require.NoError(t, err)
	assert.NotNil(t, rp)
}
...
func TestModule_RelyingPartyClientSecret(t *testing.T) {
	...
	srv := newDiscoveryServer(t)
	rp, err := m.RelyingParty(t.Context(), oidcFor(srv.URL, "cid"), "https://app.example.com/callback")
	require.NoError(t, err)
	assert.NotNil(t, rp)
}
...
func TestModule_ResourceServerUnknownType(t *testing.T) {
	...
	_, err := m.ResourceServer(t.Context(), "client-id", "https://issuer.example.com")
	require.Error(t, err)
}
